{% macro scd_type2_default_catchup_suk(
    src_table=none, 
    surrogate_key=none,
    time_column=none,
    exec_date=none,
...
WITH base_incoming AS (
    SELECT
        ...
        , TO_TIMESTAMP('9999-12-31 00:00:00', 'yyyy-MM-dd HH:mm:ss') AS meta_valid_to
...
incoming_update AS (
    ...
    SELECT
        xxhash64(
          ...
          {%- if table_columns | length > 0 -%},{%- endif -%}
          ...
          date_format(meta_valid_to,   'yyyy-MM-dd HH:mm:ss'),
          meta_is_current
        ) AS {{ surrogate_key }},
